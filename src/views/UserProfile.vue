<template>
  ...
  <div
    class=" mt-8  py-4 flex flex-col gap-4 justify-between items-center border rounded shadow-xl mx-4  "
    v-if="profile"
  >
    ...
    <div>
      <img
        class="w-32 h-32 rounded-full border-2 border-turkis "
        :src="profile.avatarUrl"
      >
    </div>
    <h2 class="text-2xl font-bold ">{{profile.userName}} aaa </h2>
...
export default {
  props: ["id", "nama", "profile"],

  async mounted() {
    const { user } = getUser();
    const { error, document } = getUserProfile("profiles", user.value.uid);
  ...
  setup() {
    ...
    //   return (
    //     profile.value && user.value && user.value.uid == profile.value.userId
    //   );
    // });
